...
export class GradientLinesComponent implements OnInit, OnDestroy {
  ...
  private buildColorCycle(): void {
    const forwardColors = this.buildGradient(this.startColor, this.endColor, this.numberOfLines);
    const reverseColors = this.buildGradient(this.endColor, this.startColor, this.numberOfLines);
  ...
  private initializeLines(): void {
    ...
    const neededLines = Math.ceil((this.containerSize * 2) / (2 * this.lineThickness)); // Covering diagonal is ~1414px
